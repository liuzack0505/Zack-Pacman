...
static void draw_ghost_invisible_bean(Map* M, const int row, const int col);
static void draw_redirection_bean(Map* M, const int row, const int col);
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case 'S':
					...
					break;
				case 'D':
					draw_ghost_invisible_bean(M, row, col);
					break;
				case 'R':
					draw_redirection_bean(M, row, col);
					break;
				case '.':
					draw_bean(M, row, col);
					break;
...
static void draw_block_index(Map* M, const int row, const int col) {
	...
	dw = block_width / 3;
		...
		al_draw_filled_rectangle(s_x, s_y,
			e_x, e_y, al_map_rgb(M->map_r, M->map_g, M->mag_b));
...
static void draw_speed_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(72,209,204));
}
static void draw_ghost_invisible_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(186, 85, 211));
}
static void draw_redirection_bean(Map* M, const int row, const int col) {
	al_draw_filled_circle(map_offset_x + col * block_width + block_width / 2.0, map_offset_y + row * block_height + block_height / 2.0, block_width / 3.0, al_map_rgb(107, 142, 35));
}
