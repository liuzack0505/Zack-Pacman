...
static void ghost_orange_move_script_FREEDOM(Ghost* ghost, Map* M, Pacman* pacman);
...
static void ghost_red_move_script_FREEDOM(Ghost* ghost, Map* M) {
	...
	int time = ghost->objData.nextTryMove;
	
	if (ghost->objData.preMove == UP) {
		if (time == 4 && cnt!=1) {
			while (time == 4 || !ghost_movable(ghost, M, time, true)) {
				time = rand() % 4 + 1;
			}
			ghost_NextMove(ghost,time);
		}
	}
	else if (ghost->objData.preMove == DOWN) {
		if (time == 1 && cnt != 1) {
			while (time == 1 || !ghost_movable(ghost, M, time, true)) {
				time = rand() % 4+1;
			}
			ghost_NextMove(ghost, time);
		}
		...
		if (time == 2 && cnt != 1) {
			while (time == 2 || !ghost_movable(ghost, M, time, true)) {
				time = rand() % 4+1;
			}
			ghost_NextMove(ghost, time);
		}
		...
		if (time == 3 && cnt != 1) {
			while (time == 3 || !ghost_movable(ghost, M, time, true)) {
				time = rand() % 4+1;
			}
			ghost_NextMove(ghost, time);
		}
...
void ghost_orange_move_script_FREEDOM(Ghost* ghost, Map* M , Pacman* pacman) {
...
void ghost_red_move_script_BLOCKED(Ghost* ghost, Map* M) {
